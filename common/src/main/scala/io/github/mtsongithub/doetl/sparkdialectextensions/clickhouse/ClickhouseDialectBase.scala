...
/**
 * Version-agnostic ClickHouse dialect logic shared by every supported Spark version.
 * Spark-version specific implementations live in the `sparkXY` sub-packages and extend this
 * class, overriding only what differs between Spark versions (see the `spark41` implementation,
 * which adds `isObjectNotFoundException`). This keeps the actual dialect logic in a single place.
 */
private[clickhouse] abstract class ClickhouseDialectBase extends JdbcDialect {
